...
def load_manifest(path: Path, os_name: str, arch: str, channel: str) -> SdkCandidate:
    data = json.loads(path.read_text(encoding="utf-8"))
    prefix = sdk_prefix(os_name, arch)
    wanted_version = version_prefix(channel)
    ...
    for item in data if isinstance(data, list) else data.get("files", []):
        ...
        if name.startswith(prefix) and wanted_version in version and url:
...
def devrepo_latest(os_name: str, arch: str, channel: str, token: str) -> SdkCandidate:
    prefix = sdk_prefix(os_name, arch)
    wanted_version = version_prefix(channel)
    ...
    for item in files:
        ...
        if name.startswith(prefix) and wanted_version in version and download_url:
...
def resolve(args: argparse.Namespace) -> SdkCandidate:
    if args.url:
        return explicit_candidate(args.url)
    channel = normalize_channel(args.channel)
    if args.manifest:
        return load_manifest(Path(args.manifest), args.os, args.arch, channel)
    token = os.environ.get(TOKEN_ENV, "")
    if token:
        return devrepo_latest(args.os, args.arch, channel, token)
    env_url = os.environ.get("CANGJIE_SDK_URL", "")
    if env_url:
        return explicit_candidate(env_url)
    raise SystemExit(
