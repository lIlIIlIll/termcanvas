...
The canonical verification toolchain is Cangjie STS `1.1.3` with cjpm `1.1.3`. CI resolves the `sts` channel at that exact version, and local gates reject any compiler mismatch. This verification pin is separate from manifests' `cjc-version = "1.1.0"` language compatibility declaration and does not claim ABI compatibility with other compilers. Official verification uses a toolchain-specific target namespace; moving a local `daily` symlink cannot change the accepted compiler or reuse objects from another SDK.
