...
Stateful widgets keep state in small public state holders such as `ListState`, `TableState`, `TextBuffer`, `Paginator`, or widget fields.
...
For app-level composition, implement `Component` when a panel needs both rendering and event handling. Register components in `EventRouter` to get global key handling, Tab/Shift-Tab focus movement, and focused event dispatch.
