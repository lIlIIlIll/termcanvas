...
- Layout and framing: `Block`, `Paragraph`, `Tabs`, `StatusBar`, `Modal`, `Viewport`.
...
- Support widgets: `HelpView`, `DebugOverlay`, `ProfilerOverlay`.
...
`ProfilerOverlay` renders `ComponentProfiler` snapshots with hottest components, update/render/timer/async counts, pending datasource state, last errors, dirty estimates, and skipped renders. Use it for component-level dashboard diagnosis; keep `DebugOverlay` for frame-level app metrics.
...
`VirtualTable` can sort by a column, filter rows by text, track selected source-row indices, style the selected cell, and keep the selected column visible. Its unsorted and unfiltered path reads only viewport rows, making it the preferred table/list shape for large row sets when the app can provide rows by index. It exposes `dirtyHeader`, `dirtyBody`, `dirtyRows`, `dirtySelectionChange`, `dirtyStatus`, and row-cache helpers for component-level dirty rendering. `Tree` supports id-based lookup and expansion helpers. `FilePicker` exposes selected paths, parent navigation, directory entry, and extension filtering that keeps directories visible.

`ListComponent`, `InputComponent`, and `TextAreaComponent` wrap stateful widgets for `ComponentHost`. They reuse the widget dirty helpers so key handling can invalidate only selected rows, input lines, or text viewports.
...
`SplitPane`, `Wizard`, `Accordion`, `Breadcrumb`, `TreeTable`, `FileDialog`, `ConfirmDialog`, and `ColorPicker` support common app workflows.
